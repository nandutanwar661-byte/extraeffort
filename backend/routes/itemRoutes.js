...
router.post('/add', upload.single('image'), async (req, res) => {
  try {
    ...
    // Agar image upload hui hai toh uska live static link add karein
    if (req.file) {
      // Localhost link ko hatakar live Render upload path set kiya hai
      parsedBody.imageUrl = `https://zoho-app.onrender.com/uploads/${req.file.filename}`;
    }
